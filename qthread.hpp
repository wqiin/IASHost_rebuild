...
namespace qthread {
	...
	struct PriorityPolicy {
		...
		*/
		struct TaskWrapper {
            ...
            //使用这个方法，使得插入的任务在优先级队列中为有序，且按照高优先级在前，
	...
	template <typename T, typename Policy = FiFoPolicy> class ConcurrentQueue {
	  public:
    	...
    	*/
        ...
        inline void push(T item, PriorityPolicy::Priority prio);//oerverload the functionality with priority symbol
	  ...
	  private:
        ...
        //1.因为编译器 不知道 Container 是模板还是普通类型,必须显式告诉它：Policy::template Container<T>
        //2.Policy::template Container<T>是依赖类型（dependent type），编译器无法确定它是：
        //类型？ 静态成员？ 枚举？所以必须写：typename, 告诉编译器后面这个是一个类型
	};
	...
	* @tparam Policy Scheduling policy (default: FiFoPolicy)
	...
	template <typename Policy = FiFoPolicy> class ThreadPool {
	  ...
	  private:
        ...
        /*
         * 如果 Policy 是 PriorityPolicy，
            那么 TaskType = PriorityPolicy::TaskWrapper；
            否则 TaskType = std::function<void()>。
	...
	*/
    ...
    template <typename Policy>
    inline ThreadPool<Policy>::~ThreadPool() {
		shutdown();
	}

	template <typename Policy> inline void ThreadPool<Policy>::worker_loop() {
		while (!stopFlag_) {
            auto taskOpt = taskQueue_.pop();//it will block here when no task inside
